...
The merge, for an aircraft already in the snapshot, is three independent
rules rather than one blanket one:

  1. raw[<module>] — each key in the incoming raw replaces that key and
     nothing else, unconditionally. Every other module's payload is left
     alone, so several sources can report on one aircraft side by side.
  2. location, direction, last_seen — taken only if the incoming last_seen
     is newer than the one held, and then as a set. Two chains observing
     the same aircraft will regularly race, and the loser is carrying an
     older position, not a better one. A position is only meaningful as a
     complete set, so its blanks are never filled from the older record.
  3. Everything else (route, airframe, airline, meta.squawk, meta.icao_hex)
     — no date check. Any value the incoming record has replaces the one
     held; a blank incoming value is skipped. A later, fuller report can
     therefore correct an earlier one; two sources that disagree will flap,
     with whoever last wrote real data winning.

first_seen sits outside all three: it is the earliest time the aircraft is
known to have been seen, so the earliest value offered is kept — an
incoming first_seen, or failing that its last_seen. A new record with no
first_seen of its own is stamped from its last_seen.
...
# Rule 2: taken as a set, and only from a newer observation. last_seen goes
# with them — it lives in meta, so it is named separately.
POSITION_SECTIONS = ("location", "direction")
    ...
    def save(self, aircraft: list[Aircraft]) -> None:
        ...
        for incoming in aircraft:
            ...
            with self._lock(hex_code):
                ...
                sources = _sources_of(incoming)

                if existing is None:
                    if incoming.meta.first_seen is None:
                        incoming = deepcopy(incoming)
                        incoming.meta.first_seen = incoming.meta.last_seen
                    self._write_one(hex_code, incoming)
                    self.log.debug("%s: new aircraft from %s", hex_code, sources)
                    continue

                merged = merge(existing, incoming)

                if _is_newer(incoming, existing):
                    self.log.debug("%s: %s is newer — position taken", hex_code, sources)
                else:
                    self.log.debug("%s: %s is not newer — position kept, raw and "
                                   "known fields still merged", hex_code, sources)

                if merged.to_dict() == existing.to_dict():
                    self.log.debug("%s: nothing changed — not rewritten", hex_code)
                    continue
...
def merge(existing: Aircraft, incoming: Aircraft) -> Aircraft:
    """
    Fold an observation into the record already held, by the three rules
    in the module docstring. They are applied independently: an older
    observation still contributes its raw payload and its known fields, it
    just does not move the aircraft.

    Returns a new Aircraft — neither argument is modified.
    """
    merged = deepcopy(existing)

    # Rule 1: per-module raw, unconditionally.
    for module_key, payload in incoming.raw.items():
        merged.raw[module_key] = deepcopy(payload)

    # Rule 2: the position set, gated on last_seen.
    if _is_newer(incoming, existing):
    ...
    # Rule 3: everything else, known values overwrite. Walked from the
    # schema rather than listed, so a section added later lands here.
    for f in fields(Aircraft):
        if f.name in POSITION_SECTIONS or f.name == "raw":
            continue
        skip = ("last_seen", "first_seen") if f.name == "meta" else ()
        _take_known(getattr(merged, f.name), getattr(incoming, f.name), skip)

    # first_seen: the earliest on offer.
    offered = [t for t in (existing.meta.first_seen,
                           incoming.meta.first_seen or incoming.meta.last_seen)
               if t is not None]
    if offered:
        merged.meta.first_seen = min(offered, key=ensure_utc)

    return merged


def _take_known(target: object, source: object, skip: tuple[str, ...] = ()) -> None:
    """
    Copy every non-UNKNOWN source value over target's, leaving the rest.

    Recurses into nested schema objects (Route.origin, Route.destination), so
    a transform that learns only the destination's country sets that one
    ...
    for f in fields(target):  # type: ignore[arg-type]
        ...
        if is_dataclass(current) and is_dataclass(incoming):
            _take_known(current, incoming)
        elif incoming is not None:
            setattr(target, f.name, deepcopy(incoming))
...
def _sources_of(aircraft: Aircraft) -> str:
    """Who an incoming record came from, for log lines — its raw keys."""
    return ", ".join(sorted(aircraft.raw)) or "an unnamed source"
