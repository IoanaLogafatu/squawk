...
- **Service** — an always-online process maintaining its own shared state
  ...
  Which services run is decided by the top-level `services` list; each may
  optionally have a `[service.<name>]` block for settings (e.g.
  `debug_level`) — a service with no block just uses its defaults.
...
using **four independent rules**, not one blanket rule:
...
4. **`meta.first_seen`** — kept as the earliest time offered, not the
   latest. On first creation of a record, set from that report's own
   `first_seen` if it supplies one, otherwise from its `last_seen`. On
   every later report, only moved *earlier*, never later — a late-arriving
   report claiming an earlier `first_seen` wins; a report claiming a later
   one is ignored. This is deliberately loose: the use case is "G-LMLC was
   in range from 12:00 to 13:20," not millisecond-accurate ATC timing, so a
   source occasionally supplying its own `first_seen` rather than just a
   `last_seen` is handled but not a scenario worth engineering further for.
...
is created outright — rules 1 through 3 are just "write everything," and
rule 4 sets `first_seen` per the creation case above.
...
10. **Console output requires a `Source` column**, computed from `raw`'s
    keys, not stored.
11. **Schema fields renamed and pruned to match the canonical block**:
    `airline` moved from `route` to the top level of `aircraft`, with
    fields renamed to `airline_name`/`airline_iata`/`airline_icao`/
    `airline_country`; `route.squawk_code` → `meta.squawk`;
    `direction.track_degrees` → `direction.heading`; `Airframe.category`,
    `Airframe.db_flags`, `Location.altitude_band`, `Location.distance_nm`,
    `Location.bearing_degrees`, and `Location.seen_seconds` removed (no
    longer in the schema — distance/bearing now live only in `raw` if a
    module wants them); `meta.first_seen` added (see rule 4 above).
    Old snapshot files with the previous field names still load; the old
    fields are simply ignored. Deletions chains run their transform list
    the same as live chains — this was always the correct reading of the
    transform-then-core ordering rule, not a behaviour change.
12. **Services gain an optional `[service.<name>]` settings block**,
    validated against a `ServiceConfig` subclass named by the service
    class (`config_class`), the same pattern chain blocks already use.
    With no block, a service uses its defaults (`debug_level = "warn"`).
    A `[service.x]` block for a service not in the `services` list is a
