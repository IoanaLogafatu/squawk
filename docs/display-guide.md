# Display Plugins — Developer Guide

A **display** is a plugin that consumes the aircraft list and writes it somewhere a human can see — a screen, a web page, a terminal, an LED strip. Mechanically, displays are not a separate type: they implement the same `BasePlugin` interface as filters and enrichments and occupy the same kind of slot in the chain. They are documented separately because they are the most common thing a Squawk user will want to build for themselves.

This guide covers the conventions, three worked examples (`console`, `http`, `epaper`), and a checklist for writing your own.

## What makes a plugin a "display"
...
- Returns its input list **unchanged**, so subsequent plugins still see the same data.
...
If you've never written a plugin before, start by copying this and swapping `print()` for your sink of choice.
...
- `SharedState` is a small thread-safe pub/sub between the plugin thread (writer) and request handler threads (readers). HTTP clients are not the plugin's problem; they read from the shared state independently.
...
E-paper screens have constraints the other examples don't: a full refresh takes ~2 seconds, panels wear with repeated writes, and the SPI bus doesn't tolerate concurrent access. The plugin interface is unchanged, but the implementation has to respect the hardware.
...
From outside, the plugin still looks like `process(aircraft) -> aircraft`. The complexity is internal — exactly as it should be.

Hardware caveat for systemd on the Pi: the service user must be in the `spi`, `gpio`, and `i2c` groups, or the plugin will work interactively but fail when run as a service.

## Writing your own — checklist

1. **Subclass `BasePlugin`** and implement `process(aircraft) -> aircraft`.
2. **Add the factory:** `def get(cfg: dict) -> YourDisplay`.
3. **Always return the list.** Even when you've done nothing else. Returning `None` breaks every plugin after you.
...
Display plugins are easy to test because `process()` is a pure function with side effects:
...
For hardware-backed displays, mock the driver at construction time — the plugin should accept a driver object via cfg (or via dependency injection in tests) rather than importing it at module level.
