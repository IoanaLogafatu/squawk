...
from services.concorde.service import ConcordeService  # noqa: E402
    ...
    return Config(
        ...
        output_chains      = {
            "display": OutputChainConfig(
                name="display", type="output_console", transform=["nop"], source="live"),
            "history": OutputChainConfig(
                name="history", type="output_console", transform=["nop"], source="deletions"),
        },
    )
...
@pytest.fixture
def concorde(app: Config) -> ConcordeService:
    """
    The Concorde service, having published once.

    Tests call refresh() and write_state() themselves rather than start
    its loop — the loop is only those two calls and a sleep.
    """
    service = ConcordeService("concorde", app.service("concorde"), app)
    service.write_state(service.refresh())
    return service
...
def make_aircraft():
    ...
    datetime arithmetic. source names the reporting module, and becomes
    the aircraft's one raw key, as it would from a real ingestor.
    """
    def _make(
        hex_code: str = "400F6A",
        ...
        **fields,
        ...
        aircraft = Aircraft(
            ...
            raw={source: {"age_seconds": age_seconds}},
        )
