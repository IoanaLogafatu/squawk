...
polls — position, altitude, speed, heading — because watching those move
is how you tell the pipeline is alive.

The SOURCE column lists the modules that have reported the aircraft. It is
worked out from raw's keys as each line is printed rather than stored
anywhere, since raw already holds one key per contributing module.

Serves both output sources: send() for a live chain, on_deleted() for a
deletions chain.
...
HEADER = (
    f"{'HEX':<8} {'CALLSIGN':<10} {'REG':<8} {'TYPE':<5} {'SQWK':<4} "
    f"{'LAT':>10} {'LON':>11} {'ALT':>7} {'SPD':>6} {'HDG':>5} {'V/S':>7}  {'ROUTE':<9} SOURCE"
)
...
def _format(one: Aircraft) -> str:
    ...
    return (
        ...
        f"{one.meta.squawk or '-':<4} "
        f"{_num(loc.latitude, 4):>10} "
        f"{_num(loc.longitude, 4):>11} "
        f"{_num(loc.altitude_feet, 0):>7} "
        f"{_num(dirn.ground_speed_knots, 0):>6} "
        f"{_num(dirn.heading, 0):>5} "
        f"{_num(dirn.vertical_rate_fpm, 0):>7}  "
        f"{origin + '->' + destination:<9} "
        f"{sources(one)}"
    )


def sources(one: Aircraft) -> str:
    """Every module that has reported this aircraft, from raw's keys."""
    return ", ".join(sorted(one.raw)) or "-"
