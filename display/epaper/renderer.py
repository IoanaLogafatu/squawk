...
def render(aircraft: Optional[Aircraft]) -> Image.Image:
    ...
    # Row 1 (y=32): aircraft type / description
    if typ:
        if len(typ) > 22:
            typ = typ[:21] + "…"
    ...
    if op:
        if len(op) > 35:
            op = op[:34] + "…"
        draw.text((8, 52), op, font=sml, fill=0)

    # Row 3 (y=68): distance + cardinal bearing
    if dist is not None:
        dist_str = f"{dist:.1f} nm {card}" if card else f"{dist:.1f} nm"
        draw.text((8, 68), dist_str, font=med, fill=0)

    # Row 4 (y=88): altitude
    if alt:
        draw.text((8, 88), alt, font=med, fill=0)
